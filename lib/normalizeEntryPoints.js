...
function getEntry(entryPointFilename, singleton, entryName) {
  var name = entryName
  var output = null
  if (!name && singleton) {
    name = 'main'
...
function getFromObject(webpackConfig) {
  ...
  for (var commonsEntry in commonsFilenames) {
    if (commonsFilenames.hasOwnProperty(commonsEntry)) {
      ...
      var result = getEntry(vendorFilename, false, commonsEntry)
  ...
  for (var entryName in entryConfig) {
    if (entryConfig.hasOwnProperty(entryName) && !commonsFilenames[entryName]) {
      var result = getEntry(entryConfig[entryName], false, entryName)
